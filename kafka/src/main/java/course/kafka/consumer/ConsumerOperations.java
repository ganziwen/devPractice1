...
public class ConsumerOperations {
    ...
    public void testConsumerForCommit() {
        ...
        while (true) {
             ...
             *
             * 这后面一般是跟着实际的业务场景:
             * 1.收到了消息,需要处理的内容封装完成之后,存入 Mysql
             * 2.入库时发生失败,Mysql 抖动或者是网络,冲突等问题.需要重试
             * 3.此时我们判断入库失败,int effectRows = 0; 这个时候就不调用 consumer.commitAsync()
             * int effectRows = orderTable.insert(OrderDo)
             * if(effectRows > 0){
             * kafkaConsumer.commitAsync(); 入库成功了,业务处理成功了,此时手动调用提交
             * }
             */
